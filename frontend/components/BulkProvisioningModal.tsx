...
const STEP_ICON: Record<StepStatus, React.ReactNode> = {
  pending: (
    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-zinc-700 text-zinc-600 text-xs">·</span>
  ),
  loading: (
    ...
    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-900 text-emerald-400 text-xs font-bold">✓</span>
  ),
  error: (
    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-red-900 text-red-400 text-xs font-bold">✕</span>
  ),
}
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="ml-1.5 rounded border border-zinc-700 bg-zinc-800 px-1.5 py-0.5 text-xs text-zinc-300 transition-all hover:border-zinc-600 hover:bg-zinc-700 active:scale-95"
    >
      {copied ? 'Copied!' : 'Copy'}
    </button>
  )
...
export default function BulkProvisioningModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="animate-scale-in flex w-full max-w-2xl flex-col rounded-xl border border-zinc-700/60 bg-zinc-900 shadow-2xl" style={{ maxHeight: '90vh' }}>
        ...
        <div className="flex-1 overflow-y-auto p-6 pt-4">
          ...
          {hasPrepSteps && prepSteps.length > 0 && (
            <div className="mb-5">
              ...
              <div className="flex flex-col gap-2 rounded-xl border border-zinc-800 bg-zinc-800/40 p-3">
                {prepSteps.map((s) => (
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button
                  ...
                  className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1 text-xs font-medium text-zinc-200 transition-all hover:border-zinc-600 hover:bg-zinc-700 active:scale-95"
                >
                  Download CSV
                </button>
              </div>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {sortedCredentials.map((c) => (
                        <tr key={c.vm_index} className="border-t border-zinc-800/60">
        ...
        {(isDone || hasFatal) && onClose && (
          <div className="shrink-0 border-t border-zinc-800 p-4">
            <button
              onClick={onClose}
              className="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-all hover:bg-indigo-500 active:scale-[0.98]"
            >
